...
static struct k_timer *timers[]
	= {[LED0] = &led0_timer, [LED1] = &led1_timer, [LED2] = &led2_timer, [LED3] = &led3_timer};
...
static void blinky_main()
{
	...
	for (;;)
	{
		...
		k_timer_start(timers[msg.led], K_MSEC(msg.blink_period_ms), K_MSEC(msg.blink_period_ms));
	}
}
...
K_THREAD_DEFINE(
	blinky_thread, 1024, blinky_main, NULL, NULL, NULL, K_LOWEST_APPLICATION_THREAD_PRIO, 0, 0
);
